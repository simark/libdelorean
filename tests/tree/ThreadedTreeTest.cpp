...
class ThreadedTreeTest : public CppUnit::TestFixture
{
	CPPUNIT_TEST_SUITE( ThreadedTreeTest );
...
private:
	...
	void superWrite(OutHistoryTree* outTree){
		for(unsigned int i = 0; i < 10000; i++){
			outTree->addInterval(AbstractInterval::SharedPtr(new NullInterval(i, i+10, (i+i+10)/2)));
		}
	}
...
public:
	void setUp()
	{			
		oht = new ThreadedOutHistoryTree(HistoryTreeConfig());
	...
	void testConcurrentReadWrite()
	{
		...
		bool success;
		boost::thread outThread(boost::bind(&ThreadedTreeTest::superWrite, this, ht));
		boost::thread inThread(boost::bind(&ThreadedTreeTest::superRead, this, ht, &success));
...
CPPUNIT_TEST_SUITE_REGISTRATION( ThreadedTreeTest );
